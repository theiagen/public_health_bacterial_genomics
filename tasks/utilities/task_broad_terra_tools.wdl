...
task export_taxon_tables {
  ...
  command <<<
    ...
    if [ ! -z ${sample_table} ]; then
       ...
       ## header
      echo -e "entity:${sample_table}_id\treads\tread1\tread2\tread1_clean\tread2_clean\trun_id\tcollection_date\toriginating_lab\tcity\tcounty\tzip\ttheiaprok_illumina_pe_version\ttheiaprok_illumina_pe_analysis_date\ttheiaprok_illumina_se_version\ttheiaprok_illumina_se_analysis_date\tseq_platform\tnum_reads_raw1\tnum_reads_raw2\tnum_reads_raw_pairs\tfastq_scan_version\tnum_reads_clean1\tnum_reads_clean2\tnum_reads_clean_pairs\ttrimmomatic_version\tfastp_version\tbbduk_docker\tr1_mean_q_raw\tr2_mean_q_raw\tr1_mean_q_clean\tr2_mean_q_clean\tassembly_fasta\tcontigs_gfa\tshovill_pe_version\tshovill_se_version\tquast_report\tquast_version\tassembly_length\tnumber_contigs\tn50_value\tcg_pipeline_report_raw\tcg_pipeline_report_clean\tcg_pipeline_docker\test_coverage_raw\test_coverage_clean\tgambit_report\tgambit_predicted_taxon\tgambit_predicted_taxon_rank\tgambit_closest_genomes\tgambit_version\tgambit_db_version\tgambit_docker\tbusco_version\tbusco_database\tbusco_results\tbusco_report\tts_mlst_results\tts_mlst_predicted_st\tts_mlst_pubmlst_scheme\tts_mlst_novel_alleles\tts_mlst_version\tserotypefinder_report\tserotypefinder_docker\tserotypefinder_serotype\tectyper_results\tectyper_version\tectyper_predicted_serotype\tshigatyper_predicted_serotype\tshigatyper_ipaB_presence_absence\tshigatyper_notes\tshigatyper_hits_tsv\tshigatyper_summary_tsv\tshigatyper_version\tshigatyper_docker\tshigeifinder_report\tshigeifinder_docker\tshigeifinder_version\tshigeifinder_ipaH_presence_absence\tshigeifinder_num_virulence_plasmid_genes\tshigeifinder_cluster\tshigeifinder_serotype\tshigeifinder_O_antigen\tshigeifinder_H_antigen\tshigeifinder_notes\tsonneityping_mykrobe_report_csv\tsonneityping_mykrobe_report_json\tsonneityping_final_report_tsv\tsonneityping_mykrobe_version\tsonneityping_mykrobe_docker\tsonneityping_species\tsonneityping_final_genotype\tsonneityping_genotype_confidence\tsonneityping_genotype_name\tlissero_results\tlissero_version\tlissero_serotype\tsistr_results\tsistr_allele_json\tsister_allele_fasta\tsistr_cgmlst\tsistr_version\tsistr_predicted_serotype\tseqsero2_report\tseqsero2_version\tseqsero2_predicted_antigenic_profile\tseqsero2_predicted_serotype\tseqsero2_predicted_contamination\tkleborate_output_file\tkleborate_version\tkleborate_docker\tkleborate_key_resistance_genes\tkleborate_genomic_resistance_mutations\tkleborate_mlst_sequence_type\tkleborate_klocus\tkleborate_ktype\tkleborate_olocus\tkleborate_otype\tkleborate_klocus_confidence\tkleborate_olocus_confidence\tkaptive_version\tkaptive_output_file_k\tkaptive_output_file_oc\tkaptive_k_locus\tkaptive_k_type\tkaptive_kl_confidence\tkaptive_oc_locus\tkaptive_ocl_confidence\tabricate_abaum_plasmid_tsv\tabricate_abaum_plasmid_type_genes\tabricate_database\tabricate_version\tabricate_docker\tlegsta_results\tlegsta_predicted_sbt\tlegsta_version\ttbprofiler_output_file\ttbprofiler_output_bam\ttbprofiler_output_bai\ttbprofiler_version\ttbprofiler_main_lineage\ttbprofiler_sub_lineage\ttbprofiler_dr_type\ttbprofiler_resistance_genes\tamrfinderplus_all_report\tamrfinderplus_amr_report\tamrfinderplus_stress_report\tamrfinderplus_virulence_report\tamrfinderplus_version\tamrfinderplus_db_version\tamrfinderplus_amr_genes\tamrfinderplus_stress_genes\tamrfinderplus_virulence_genes\tamrfinderplus_amr_classes\tamrfinderplus_amr_subclasses\tgenotyphi_report_tsv\tgenotyphi_mykrobe_json\tgenotyphi_version\tgenotyphi_species\tgenotyphi_st_probes_percent_coverage\tgenotyphi_final_genotype\tgenotyphi_genotype_confidence\tani_highest_percent\tani_highest_percent_bases_aligned\tani_output_tsv\tani_top_species_match\tani_mummer_version\tresfinder_pheno_table\tresfinder_pheno_table_species\tresfinder_seqs\tresfinder_results\tresfinder_pointfinder_pheno_table\tresfinder_pointfinder_results\tresfinder_db_version\tresfinder_docker\tprokka_gff\tprokka_gbk\tprokka_sqn\tplasmidfinder_plasmids\tplasmidfinder_results\tplasmidfinder_seqs\tplasmidfinder_docker\tplasmidfinder_db_version\tpbptyper_predicted_1A_2B_2X\tpbptyper_pbptype_predicted_tsv\tpbptyper_version\tpbptyper_docker\tpoppunk_gps_cluster\tpoppunk_gps_external_cluster_csv\tpoppunk_GPS_db_version\tpoppunk_version\tpoppunk_docker\tseroba_version\tseroba_docker\tseroba_serotype\tseroba_ariba_serotype\tseroba_ariba_identity\tseroba_details\tmidas_docker\tmidas_report\tmidas_primary_genus\tmidas_secondary_genus\tmidas_secondary_genus_abundance\tbakta_gbff\tbakta_gff3\tbakta_tsv\tbakta_summary\tbakta_version\tpasty_serogroup\tpasty_serogroup_coverage\tpasty_serogroup_fragments\tpasty_summary_tsv\tpasty_blast_hits\tpasty_all_serogroups\tpasty_version\tpasty_docker\tpasty_comment\tqc_check\tqc_standard" > ~{samplename}_terra_table.tsv
      ## TheiaProk Outs
      echo -e "~{samplename}\t~{reads}\t~{read1}\t~{read2}\t~{read1_clean}\t~{read2_clean}\t~{run_id}\t~{collection_date}\t~{originating_lab}\t~{city}\t~{county}\t~{zip}\t~{theiaprok_illumina_pe_version}\t~{theiaprok_illumina_pe_analysis_date}\t~{theiaprok_illumina_se_version}\t~{theiaprok_illumina_se_analysis_date}\t~{seq_platform}\t~{num_reads_raw1}\t~{num_reads_raw2}\t~{num_reads_raw_pairs}\t~{fastq_scan_version}\t~{num_reads_clean1}\t~{num_reads_clean2}\t~{num_reads_clean_pairs}\t~{trimmomatic_version}\t~{fastp_version}\t~{bbduk_docker}\t~{r1_mean_q_raw}\t~{r2_mean_q_raw}\t~{r1_mean_q_clean}\t~{r2_mean_q_clean}\t~{assembly_fasta}\t~{contigs_gfa}\t~{shovill_pe_version}\t~{shovill_se_version}\t~{quast_report}\t~{quast_version}\t~{assembly_length}\t~{number_contigs}\t~{n50_value}\t~{cg_pipeline_report_raw}\t~{cg_pipeline_report_clean}\t~{cg_pipeline_docker}\t~{est_coverage_raw}\t~{est_coverage_clean}\t~{gambit_report}\t~{gambit_predicted_taxon}\t~{gambit_predicted_taxon_rank}\t~{gambit_closest_genomes}\t~{gambit_version}\t~{gambit_db_version}\t~{gambit_docker}\t~{busco_version}\t~{busco_database}\t~{busco_results}\t~{busco_report}\t~{ts_mlst_results}\t~{ts_mlst_predicted_st}\t~{ts_mlst_pubmlst_scheme}\t~{ts_mlst_novel_alleles}\t~{ts_mlst_version}\t~{serotypefinder_report}\t~{serotypefinder_docker}\t~{serotypefinder_serotype}\t~{ectyper_results}\t~{ectyper_version}\t~{ectyper_predicted_serotype}\t~{shigatyper_predicted_serotype}\t~{shigatyper_ipaB_presence_absence}\t~{shigatyper_notes}\t~{shigatyper_hits_tsv}\t~{shigatyper_summary_tsv}\t~{shigatyper_version}\t~{shigatyper_docker}\t~{shigeifinder_report}\t~{shigeifinder_docker}\t~{shigeifinder_version}\t~{shigeifinder_ipaH_presence_absence}\t~{shigeifinder_num_virulence_plasmid_genes}\t~{shigeifinder_cluster}\t~{shigeifinder_serotype}\t~{shigeifinder_O_antigen}\t~{shigeifinder_H_antigen}\t~{shigeifinder_notes}\t~{sonneityping_mykrobe_report_csv}\t~{sonneityping_mykrobe_report_json}\t~{sonneityping_final_report_tsv}\t~{sonneityping_mykrobe_version}\t~{sonneityping_mykrobe_docker}\t~{sonneityping_species}\t~{sonneityping_final_genotype}\t~{sonneityping_genotype_confidence}\t~{sonneityping_genotype_name}\t~{lissero_results}\t~{lissero_version}\t~{lissero_serotype}\t~{sistr_results}\t~{sistr_allele_json}\t~{sister_allele_fasta}\t~{sistr_cgmlst}\t~{sistr_version}\t~{sistr_predicted_serotype}\t~{seqsero2_report}\t~{seqsero2_version}\t~{seqsero2_predicted_antigenic_profile}\t~{seqsero2_predicted_serotype}\t~{seqsero2_predicted_contamination}\t~{kleborate_output_file}\t~{kleborate_version}\t~{kleborate_docker}\t~{kleborate_key_resistance_genes}\t~{kleborate_genomic_resistance_mutations}\t~{kleborate_mlst_sequence_type}\t~{kleborate_klocus}\t~{kleborate_ktype}\t~{kleborate_olocus}\t~{kleborate_otype}\t~{kleborate_klocus_confidence}\t~{kleborate_olocus_confidence}\t~{kaptive_version}\t~{kaptive_output_file_k}\t~{kaptive_output_file_oc}\t~{kaptive_k_locus}\t~{kaptive_k_type}\t~{kaptive_kl_confidence}\t~{kaptive_oc_locus}\t~{kaptive_ocl_confidence}\t~{abricate_abaum_plasmid_tsv}\t~{abricate_abaum_plasmid_type_genes}\t~{abricate_database}\t~{abricate_version}\t~{abricate_docker}\t~{legsta_results}\t~{legsta_predicted_sbt}\t~{legsta_version}\t~{tbprofiler_output_file}\t~{tbprofiler_output_bam}\t~{tbprofiler_output_bai}\t~{tbprofiler_version}\t~{tbprofiler_main_lineage}\t~{tbprofiler_sub_lineage}\t~{tbprofiler_dr_type}\t~{tbprofiler_resistance_genes}\t~{amrfinderplus_all_report}\t~{amrfinderplus_amr_report}\t~{amrfinderplus_stress_report}\t~{amrfinderplus_virulence_report}\t~{amrfinderplus_version}\t~{amrfinderplus_db_version}\t~{amrfinderplus_amr_genes}\t~{amrfinderplus_stress_genes}\t~{amrfinderplus_virulence_genes}\t~{amrfinderplus_amr_classes}\t~{amrfinderplus_amr_subclasses}\t~{genotyphi_report_tsv}\t~{genotyphi_mykrobe_json}\t~{genotyphi_version}\t~{genotyphi_species}\t~{genotyphi_st_probes_percent_coverage}\t~{genotyphi_final_genotype}\t~{genotyphi_genotype_confidence}\t~{ani_highest_percent}\t~{ani_highest_percent_bases_aligned}\t~{ani_output_tsv}\t~{ani_top_species_match}\t~{ani_mummer_version}\t~{resfinder_pheno_table}\t~{resfinder_pheno_table_species}\t~{resfinder_seqs}\t~{resfinder_results}\t~{resfinder_pointfinder_pheno_table}\t~{resfinder_pointfinder_results}\t~{resfinder_db_version}\t~{resfinder_docker}\t~{prokka_gff}\t~{prokka_gbk}\t~{prokka_sqn}\t~{plasmidfinder_plasmids}\t~{plasmidfinder_results}\t~{plasmidfinder_seqs}\t~{plasmidfinder_docker}\t~{plasmidfinder_db_version}\t~{pbptyper_predicted_1A_2B_2X}\t~{pbptyper_pbptype_predicted_tsv}\t~{pbptyper_version}\t~{pbptyper_docker}\t~{poppunk_gps_cluster}\t~{poppunk_gps_external_cluster_csv}\t~{poppunk_GPS_db_version}\t~{poppunk_version}\t~{poppunk_docker}\t~{seroba_version}\t~{seroba_docker}\t~{seroba_serotype}\t~{seroba_ariba_serotype}\t~{seroba_ariba_identity}\t~{seroba_details}\t~{midas_docker}\t~{midas_report}\t~{midas_primary_genus}\t~{midas_secondary_genus}\t~{midas_secondary_genus_abundance}\t~{bakta_gbff}\t~{bakta_gff3}\t~{bakta_tsv}\t~{bakta_summary}\t~{bakta_version}\t~{pasty_serogroup}\t~{pasty_serogroup_coverage}\t~{pasty_serogroup_fragments}\t~{pasty_summary_tsv}\t~{pasty_blast_hits}\t~{pasty_all_serogroups}\t~{pasty_version}\t~{pasty_docker}\t~{pasty_comment}\t~{qc_check}\t~{qc_standard}"  >> ~{samplename}_terra_table.tsv
