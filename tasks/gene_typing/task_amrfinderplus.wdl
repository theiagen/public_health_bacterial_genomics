...
task amrfinderplus_nuc {
  ...
  command <<<
    ...
    amr_classes=$(awk -F '\t' 'BEGIN{OFS=":"} {print $7,$12}' 20-220-0294_amrfinder_amr.tsv | tail -n+2 | tr '\n' ', ' | sed 's/.$//')
    # create string outputs for AMR drug subclasses
    amr_subclasses=$(awk -F '\t' 'BEGIN{OFS=":"} {print $7,$13}' 20-220-0294_amrfinder_amr.tsv | tail -n+2 | tr '\n' ', ' | sed 's/.$//')
