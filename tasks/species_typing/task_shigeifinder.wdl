...
task shigeifinder {
  ...
  command <<<
    ...
    echo "checking for shigeifinder dependencies..."
    shigeifinder --check

    # run shigeifinder on assembly
    ...
    head -n 2 ~{samplename}_shigeifinder.tsv | tail -n 1 | cut -f 3 >shigeifinder_virulence_plasmid_type.txt
    ...
    if [ "$(cat shigeifinder_virulence_plasmid_type.txt)" == "" ]; then
       echo "ShigEiFinder virulence_plasmid field was empty" > shigeifinder_virulence_plasmid_type.txt
    fi
  ...
  output {
    ...
    String shigeifinder_virulence_plasmid_type = read_string("shigeifinder_ipaH_presence_absence.txt")
  }
  runtime {
    docker: "~{docker}"
    memory: "8 GB"
